...
function tof_map = tof_picker_AIC(receive_data, time_hist, speed_in_water, ring_diameter)
    ...
    window_size_half = 128;                                         % AIC half window size. two times of it are same to search range.
 ...
 %   AIC_temp = AIC;
    ...
    for transmit_i = 1:transmit_num
        ...
        scan_region_start = round((tof_water_rotated/dt).*0.8) + window_size_half - window_start;   % search region start positon in window data
        scan_region_end = round((tof_water_rotated/dt).*1.2) + window_size_half - window_start+1;   % search region end position in window data 
 ...
 %           AIC_temp(AIC_i,:,transmit_i) = AIC_i*log(S1) + (2*window_size_half-AIC_i-1)*log(S2); 
        ...
        [AIC_min,p] =  n min(AIC(:,:,transmit_i));                                     % find min AIC value position in window for each receive element
