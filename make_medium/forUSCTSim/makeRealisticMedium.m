...
if (1<= location) &&  (location<= 3)
    cd('\\Azlab-fs01\東研究室\個人work\竹内(ひ)\data\pictures_of_calf\main_data\2018_07_10');
    I = imread('IMG_2918.jpg');
elseif location>=4
    cd('\\Azlab-fs01\東研究室\個人work\竹内(ひ)\data\pictures_of_calf\main_data\2018_08_28');
...
if location == 1
    ...
    rateEMCL = (sum(sum(bwImg(Nx/2-ROI.Nx/2+1:Nx/2+ROI.Nx/2,:))) / numel(bwImg(Nx/2-ROI.Nx/2+1:Nx/2+ROI.Nx/2,:))) * 100;
    medium.density = ones(Ny, Nx) * baseDensity;
    medium.sound_speed = ones(Ny, Nx) * baseSoundSpeed;
