function [ medium ] = makePointMedium( param, kgrid, rateIMCL,  x_coordinate, y_coordinate)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 点媒質を作成する．
...
%点媒質の数を求める
num_PointMedium = length(x_coordinate);
%% 点媒質設定
for ii = 1:num_PointMedium
    tmp_x = find(kgrid.x_vec == x_coordinate(ii)/1e3);
    tmp_y = find(kgrid.y_vec == y_coordinate(ii)/1e3);
    medium.sound_speed(tmp_y,tmp_x) = v_fat;
    medium.density(tmp_y,tmp_x) = den_fat;
end
end
