function [ Medium, Layer ] = make_layered_medium_horizontal( thickness, Grid )
...
for j = 1:length(Grid.y)
    if abs(Grid.y(j))<=Layer.thickness
        Medium.sound_speed(j,:) = v_fat;
        Medium.density(j,:) = den_fat;
    end
end
end