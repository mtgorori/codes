...
for ii = 1:t_num
    ...
    for jj = 1:t_num
        if ii == jj
            ...
            while ~isempty(p)
                %線分が属するセルの検出
                x_ind = find(abs(p_init(1,1) - x_grid) == 0);
                y_ind = find(abs(p_init(2,1) - y_grid) == 0);
                if isempty(x_ind)
                    x_ind = find(abs(p_init(1,1) - x_grid - cell_size/2) <= cell_size/2);
                end
                if isempty(y_ind)
                    y_ind = find(abs(p_init(2,1) - y_grid - cell_size/2) <= cell_size/2);
                end
                %線分のもう一方の端点の検出
                p(:,b) = [];%端点の除去
            ...
            %%%あまりきれいなやり方でないが，同じ手順で算出するものが異なるといったアプローチ
            %%%v_pre_aveのサイズが変動する現象があったのでこれで対症処置
            if length(v_pre_ave) ~=1
                v_pre_ave = 0;
                v_pre_ave = v_pre_ave(1,1);
            end 
            %初期座標設定(xの値が最小な点）
            [~,b] = min(q(1,:));
            q_init = q(:,b);
            while ~isempty(q)
                %線分が属するセルの検出
                x_ind = find(abs(q_init(1,1) - x_grid) == 0);
                y_ind = find(abs(q_init(2,1) - y_grid) == 0);
                if isempty(x_ind)
                    x_ind = find(abs(q_init(1,1) - x_grid - cell_size/2) <= cell_size/2);
                end
                if isempty(y_ind)
                    y_ind = find(abs(q_init(2,1) - y_grid - cell_size/2) <= cell_size/2);
                end
                %線分のもう一方の端点の検出
                q(:,b) = [];%端点の除去
                [~,b] = min(q(1,:));%もう一方の端点のインデックス検出
                q_neighbor = q(:,b);%もう一方の端点検出
                %寄与音速算出
                q_length = norm(q_init - q_neighbor);
                v_sim(x_ind, y_ind) = v_pre_ave * q_length / all_length;
                q_init = q_neighbor;
            end
        end
    end
