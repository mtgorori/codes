...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% time_hist = zeros(size(rfdata,1),size(rfdata,3));
% for i = 1:size(time_hist,1)
%     time_hist(i,:) = i*kgrid.dt;
% end

function tof_map = tof_picker_2(varargin)         % main function for this file.

%important parameters
speed_in_water = 1540;  %
ring_radius = 0.05;    % % 3.5cm diameter of Ring
...
if nargin == 3   % calculated one tof map
    ...
    elseif strcmp(varargin{3},'AIC')
        ...
        tic;
        tof_map = tof_picker_AIC(varargin{1}, varargin{2}, speed_in_water, ring_radius);
...
function tof_map = tof_picker_AIC(receive_data, time_hist, speed_in_water, ring_radius)

[~, receive_num, transmit_num] = size(receive_data);      % get size of received data.
rr = ring_radius;
%センサ設置
t_num = 256;%トランスデューサ数
...
t_pos(1,:) = rr*cos(t);
t_pos(2,:) = rr*sin(t);
element_dis = sqrt((rr-t_pos(1,:)).^2 + (0-t_pos(2,:)).^2);  % element distance to the first elements
...
for transmit_i = 1:transmit_num
    ...
    [~, m_r] = max(tof_water_rotated);                            % find the position of latest arrival receive signal.
    ...
    for AIC_i = 1:2*window_size_half
        ...
        scan_region_mask((AIC_i<scan_region_start) +(AIC_i> scan_region_end)) = inf;      % value of positon outof search region will be setted to be inf.
    ...
    [~,p] = min(AIC(:,:,transmit_i));                                     % find min AIC value position in window for each receive element
